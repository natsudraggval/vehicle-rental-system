...
function AdminLogin() {
  ...
  useEffect(() => {
    // Redirect if already logged in as non-admin
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const user = res.data; // expecting { token, _id, email, firstName, lastName, role }
      ...
      if (user && user.token) {
        ...
        if (user.role === 'admin') {
          navigate('/admin'); // changed from '/admin/admindashboard' to an existing route
        } else {
