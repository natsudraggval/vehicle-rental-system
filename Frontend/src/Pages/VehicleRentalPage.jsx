...
function VehicleRentalPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!user || !user._id) {
      toast.error("You must log in to book a vehicle.");
      return;
    }
  ...
  if (loading)
    return <img src="/spinner.svg" alt="Loading" className="h-15 w-15" />;
      ...
      <div className="max-w-5xl mx-auto bg-white rounded-lg shadow-lg p-6 grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <div className="bg-gray-50 p-8 rounded-2xl shadow-inner">
          ...
          >
            ...
            <button
              type="submit"
              disabled={!!dateError || totalPrice === 0 || !vehicle.isAvailable}
              className="w-full mt-1 text-white py-3 text-lg rounded-lg shadow-md bg-cyan-500 hover:bg-cyan-600 disabled:bg-gray-400"
            >
              Confirm Booking
            </button>
          </form>
